...
export default function DocumentCard({ document, showArchived, onDelete, onClick, onDuplicate, onRename, onArchive, onRestore }: DocumentCardProps) {
    ...
    <>
      <div className="group relative flex flex-col bg-zinc-900 border border-zinc-700 hover:border-zinc-500 rounded-lg shadow-sm p-6 transition-colors">
        ...
        <div className="flex items-center gap-2 mb-3">
          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${TYPE_STYLES[document.type] ?? TYPE_STYLES.OTHER}`}>
            {TYPE_LABELS[document.type] ?? "Other"}
          </span>
          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${STATUS_STYLES[document.status] ?? STATUS_STYLES.DRAFT}`}>
            {document.status}
          </span>
        </div>
        ...
        >
          ...
          <p className="mt-1 text-xs text-zinc-500">v{document.versionNumber} &middot; {formattedDate}</p>
        </button>
